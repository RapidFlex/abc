...
struct If_DsdObj_t_
{
    ...
    //added by ymc, for distribution analysis
    unsigned nOccurNormal;
...
static inline int           If_DsdVecObjDummy( Vec_Ptr_t * p, int iObj )                { return If_DsdVecObj( p, iObj )->dummy;                                             }
...
//added by ymc, for distribution analysis
static inline void If_DsdVecObjIncOccurNormal( Vec_Ptr_t * p, int iObj ) {If_DsdVecObj(p, iObj)->nOccurNormal ++;}
static inline void If_DsdVecObjIncOccurCutset( Vec_Ptr_t * p, int iObj ) {If_DsdVecObj(p, iObj)->nOccurCutset ++;}
static inline void If_DsdVecObjIncOccurCutBest( Vec_Ptr_t * p, int iObj ) {If_DsdVecObj(p, iObj)->nOccurCutBest ++;}
int If_DsdManIncOccur( If_DsdMan_t* p, int iObj, int type)
{
    if(type == 0)
        If_DsdVecObjIncOccurNormal(&p->vObjs, iObj);
    else if(type == 1)
        If_DsdVecObjIncOccurCutset(&p->vObjs, iObj);
    else if(type == 2)
        If_DsdVecObjIncOccurCutBest(&p->vObjs, iObj);
    else
        return -1;
    return 0;
}
...
//added by ymc
int set_all_obj_mark(If_DsdMan_t* pMan) // fMark == 1 means UNSAT
{
    If_DsdObj_t* pObj;
    int i;
    If_DsdVecForEachObj(&pMan->vObjs, pObj, i)
    {
        pObj->fMark = 1;
...
void print_Dsd_result(If_DsdMan_t* pMan){
    If_DsdObj_t* pObj;
    int i;
    long k=0;
    If_DsdVecForEachObj(&pMan->vObjs, pObj, i)
    {   if(pObj->fMark==0)
            k++;
        printf("pObjId  %d  , fMark  %d\n",pObj->Id,pObj->fMark);
        
    }
    printf("%ld",k);
}
int clear_all_obj_mark(If_DsdMan_t* pMan) // fMark == 1 means UNSAT
{
    If_DsdObj_t* pObj;
    int i;
    If_DsdVecForEachObj(&pMan->vObjs, pObj, i)
...
void clear_one_obj_mark(If_DsdMan_t* pMan, int iObjId)
{
    If_DsdVecObjClearMark(&pMan->vObjs, iObjId);
}

void set_obj_match_info(If_DsdMan_t* pMan, int iObjId, unsigned int matchInfo)
{   
    If_DsdObj_t* pObj = If_DsdVecObj(&pMan->vObjs, iObjId);
    pObj->dummy = matchInfo;
}

unsigned int get_obj_match_info(If_DsdMan_t* pMan, int iObjId)
{
    If_DsdObj_t* pObj = If_DsdVecObj(&pMan->vObjs, iObjId);
    return pObj->dummy;
}
...
int If_DsdManReadDummy(If_DsdMan_t * p,int iDsd)
{
    return If_DsdVecObjDummy( &p->vObjs, Abc_Lit2Var(iDsd) );
}
...
void If_DsdManPrintOne( FILE * pFile, If_DsdMan_t * p, int iObjId, unsigned char * pPermLits, int fNewLine )
{
    ...
    //modified by ymc, for distribution analysis
    //fprintf( pFile, "%8d ",   If_DsdVecObjRef(&p->vObjs, iObjId) );
    fprintf( pFile, "%8d ",   If_DsdVecObj(&p->vObjs, iObjId)->nOccurNormal );
    fprintf( pFile, "%8d ",   If_DsdVecObj(&p->vObjs, iObjId)->nOccurCutset );
    fprintf( pFile, "%8d ",   If_DsdVecObj(&p->vObjs, iObjId)->nOccurCutBest );
    //fprintf( pFile, "%8f%% ",   100.0*If_DsdVecObj(&p->vObjs, iObjId)->nOccurCutBest / If_DsdVecObj(&p->vObjs, iObjId)->nOccurCutset  );
    //ymc fprintf( pFile, "%d  ",    If_DsdVecObjMark(&p->vObjs, iObjId) );
    If_DsdManPrint_rec( pFile, p, Abc_Var2Lit(iObjId, 0), pPermLits, &nSupp );

//ymc: print truthTable
    int nVars, nWords;
    int i;
    word* pTruth;
    nVars = If_DsdObjSuppSize(If_DsdVecObj(&p->vObjs, iObjId));
    word * If_DsdManComputeTruth( If_DsdMan_t * p, int iDsd, unsigned char * pPermLits );
    pTruth = If_DsdManComputeTruth(p, Abc_Var2Lit(iObjId,0), NULL);
    nWords = Abc_TtWordNum(nVars);

    //if(If_DsdVecObj(&p->vObjs, iObjId)->nOccurCutBest > 0)
    //{
	    char pPerm[16];
	    Abc_TtHieMan_t * pMan = Abc_TtHieManStart(nVars, 5);
	    Abc_TtCanonicizeWrap(Abc_TtCanonicizeAda, pMan, pTruth, nVars, pPerm, 1199);
    ...
    {
        if(i)
            fprintf( pFile, "%016lx_", pTruth[i]);
        else
            fprintf( pFile, "%016lx", pTruth[i]);
    }

    fprintf( pFile, "\tfMark: %d",    If_DsdVecObjMark(&p->vObjs, iObjId) );
    fprintf( pFile, "\tdummy: %d",     If_DsdVecObj(&p->vObjs, iObjId)->dummy);
...
void If_DsdManPrintOccurs( FILE * pFile, If_DsdMan_t * p )
{
    ...
    If_DsdObj_t * pObj;
    Vec_Int_t * vOccurs;
    Vec_Int_t * vOccursAll; //ymc
    int nOccurs, nOccursMax, nOccursAll;
    int i, k, nSizeMax, Counter;
    int Counter2; //ymc
    // determine the largest fanin and fanout
    printf("========== For nOccurCutset ==========\n");
...
//modified by ymc, for distribution analysis
    ...
    vOccursAll  = Vec_IntStart( nSizeMax ); //ymc
...
//modified by ymc, for distribution analysis
        ...
        if ( nOccurs < 10 )
        {
            ...
            Vec_IntAddToEntry( vOccursAll, nOccurs, nOccurs );
        }
        else if ( nOccurs < 100 )
        {
            Vec_IntAddToEntry( vOccurs, 10 + nOccurs/10, 1 );
            Vec_IntAddToEntry( vOccursAll, 10 + nOccurs/10, nOccurs );
        }
        else if ( nOccurs < 1000 )
        {
            Vec_IntAddToEntry( vOccurs, 20 + nOccurs/100, 1 );
            Vec_IntAddToEntry( vOccursAll, 20 + nOccurs/100, nOccurs );
        }
        else if ( nOccurs < 10000 )
        {
            Vec_IntAddToEntry( vOccurs, 30 + nOccurs/1000, 1 );
            Vec_IntAddToEntry( vOccursAll, 30 + nOccurs/1000, nOccurs );
        }
        else if ( nOccurs < 100000 )
        {
            Vec_IntAddToEntry( vOccurs, 40 + nOccurs/10000, 1 );
            Vec_IntAddToEntry( vOccursAll, 40 + nOccurs/10000, nOccurs );
        }
        else if ( nOccurs < 1000000 )
        {
            Vec_IntAddToEntry( vOccurs, 50 + nOccurs/100000, 1 );
            Vec_IntAddToEntry( vOccursAll, 50 + nOccurs/100000, nOccurs );
        }
        else if ( nOccurs < 10000000 )
        {
            Vec_IntAddToEntry( vOccurs, 60 + nOccurs/1000000, 1 );
            Vec_IntAddToEntry( vOccursAll, 60 + nOccurs/1000000, nOccurs );
        }
        else
        {
            printf("nOccurs is too large!\n");
            Vec_IntAddToEntry( vOccurs, 60 + nOccurs/1000000, 1 );
            Vec_IntAddToEntry( vOccursAll, 60 + nOccurs/1000000, nOccurs );
        }
    }
...
//added by ymc
        fprintf( pFile, "%12d   ", Vec_IntEntry(vOccursAll, k) );
        Counter2 += Vec_IntEntry(vOccursAll, k);
        fprintf( pFile, "(%6.2f %%)", 100.0*Counter2/nOccursAll );

        fprintf( pFile, "\n" );
    }
    Vec_IntFree( vOccurs );
    Vec_IntFree( vOccursAll );
    fprintf( pFile, "Fanins: Max = %d. Ave = %.2f.\n", nOccursMax,  1.0*nOccursAll/Vec_PtrSize(&p->vObjs) );
    fprintf( pFile, "nOccursAll = %d\n", nOccursAll);

    printf("========== For nOccurCutBest ==========\n");
    nOccursMax = nOccursAll = 0;
    Counter = Counter2 = 0;
    If_DsdVecForEachNode( &p->vObjs, pObj, i )
    {
...
//Modified by ymc, for distribution analysis
        ...
        Vec_IntPush( vCounts, -(int)pObj->nOccurCutBest );
...
//    fprintf( pFile, "\n" );
    ...
    for ( i = 0; i < Abc_MinInt(Vec_IntSize(vCounts), 1000000); i++ )
    {
        printf( "%2d : ", i+1 );
